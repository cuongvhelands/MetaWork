...
$(document).on('change', ".ddlDuAnT", function () {
    var id = this.id;
    ngayLamViec = id.replace("ddlDuAn-", "");
    var duAnId = $("#" + id).val();
    var dataGiaiDoan = getDuAnThanhPhan(duAnId);
    ...
    $("#ddlDuAnTPT-" + ngayLamViec).html(str);
})
...
$(document).on("click", ".btnSubmitTD", function () {
    ...
    if (duAnId == 0) {
        check = false;
        $("#errorddlDuAnTPT-" + id).text("Bạn hãy chọn dự án thành phần.");
    } else {        
        $("#errorddlDuAnTPT-" + id).text("");
    }
    if (quyen == 3) {
...
$(document).on("change", "#ddlDuAnPop", function () {
    ...
    $("#ddlDuAnTPPop").html(str);
})
$(document).on("change", "#ddlDuAnTPPop", function () {
    var duAnId = $("#ddlDuAnTPPop").val(); 
