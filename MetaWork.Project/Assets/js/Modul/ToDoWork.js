...
$(document).on("change", "#cboDuAn", function () {
    var id = $("#cboDuAn").val();
    var dataGiaiDoan = getGiaiDoanDuAn(id);
    if (dataGiaiDoan && dataGiaiDoan.length > 0) {
        var str = '<option value="0"> Chọn giai đoạn dự án </option>';
...
$("#txtTenShipF").keyup(function (event) {
    var value = $("#txtTenShipF").val();
    var tr1 = $("#tableToDoWork td.tenCongViecF:contains('" + value + "')").parent();
    var arrayDuAn = [];
    var arrayShipId = [];
    if (tr1.length > 0) {
        $.each(tr1, function (index, value) {
            var col = value.className.split("-");
...
$(document).on('click', ".shipdetails", function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#cboDuAnDT").val(ship.DuAnId);
            changeDuAnDT(ship.DuAnId);
...
$("#btnUpdateShip").click(function () {
    ...
    ship.DuAnId = $("#cboDuAnDT").val();
    ship.GiaiDoanDuAnId = $("#cboGiaiDoanDuAnDT").val();
