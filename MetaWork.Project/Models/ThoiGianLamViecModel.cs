        ...
        public ResultViewModel StopTodo(int congViecId, Guid userId)
        {
            ...
            CongViecModel model = new CongViecModel();         
                ...
                if (thoiGian != null)
                {
                    ...
                    if (thoiGianLamViecProvider.CheckInsertTime2(startTime.Value, endtime, userId))
                    {
                        result.Status = thoiGianLamViecProvider.UpdateStopBy(congViecId, userId);                   
                    }
                    else
                    {
                        result.Status = false;
                        result.Message = "Có khoảng time trong giữa thời gian chạy time ";
                    }
                ...
                foreach (var nguoiDung in nguoiDungs)
                {
                    ...
                    if (lst != null && lst.Count > 0)
                    {
                        foreach (var item in lst)
                        {
                            ...
                            else ds.DayTypeId = item.LoaiNgayLamViec.Value;
        ...
        public ResultViewModel AddOffTime(CongViecViewModel vm, Guid userId)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        var newId = congViecProvider.InsertEntry(vm.DuAnId, vm.CongViecId, userId, vm.NguoiXuLyId.Value, vm.TenCongViec, vm.MoTa, week, vm.Nam);
                        if (newId > 0)
                        {
